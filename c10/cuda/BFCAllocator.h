...
class BFCAllocator {
 public:
  ...
  int64_t AllocationId(const void* ptr);
 ...
 private:
  ...
  struct Chunk {
    ...
    std::string DebugString(BFCAllocator* a,
                       bool recurse) {
      std::string dbg = "DebugString";
      // std::strings::StrAppend(
      //     &dbg, "  Size: ", std::strings::HumanReadableNumBytes(size),
      //     " | Requested Size: ", std::strings::HumanReadableNumBytes(requested_size),
      //     " | in_use: ", in_use());
      // if (recurse && prev != BFCAllocator::kInvalidChunkHandle) {
      //   Chunk* p = a->ChunkFromHandle(prev);
      //   std::strings::StrAppend(&dbg, ", prev: ", p->DebugString(a, false));
      // }
      // if (recurse && next != BFCAllocator::kInvalidChunkHandle) {
      //   Chunk* n = a->ChunkFromHandle(next);
      //   std::strings::StrAppend(&dbg, ", next: ", n->DebugString(a, false));
      // }
      return dbg;
    }
  };
  ...
  class RegionManager {
   ...
   private:
    ...
    const AllocationRegion* RegionFor(const void* p) const {
      ...
      std::cerr << "Could not find Region for " << p;
      return nullptr;
    }
