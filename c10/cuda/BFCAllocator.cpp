...
BFCAllocator::BFCAllocator(SubAllocator* sub_allocator, size_t total_memory,
  ...
  for (BinNum b = 0; b < kNumBins; b++) {
    ...
    // std::cerr << "Creating bin of max chunk size "
    //         << (bin_size) << "\n";
...
BFCAllocator::~BFCAllocator() {
  // Return memory back.
  std::cerr << "Number of regions allocated: "
...
bool BFCAllocator::Extend(size_t alignment, size_t rounded_bytes) {
  ...
  // std::cerr << "Extending allocation by " << bytes
  //         << " bytes.\n";

  total_region_allocated_bytes_ += bytes;
  // std::cerr << "Total allocated bytes: "
  //         << total_region_allocated_bytes_ << "\n";

  // std::cerr << "Allocated memory at " << mem_addr << " to "
...
void* BFCAllocator::AllocateRaw(size_t unused_alignment, size_t num_bytes,
  ...
  if (/*allocation_attr.no_retry_on_failure*/true) {
...
void* BFCAllocator::AllocateRawInternal(size_t unused_alignment,
  ...
  if (num_bytes == 0) {
    std::cerr << "tried to allocate 0 bytes";
    return nullptr;
  }
  ...
  if (dump_log_on_failure) {
    std::cerr << "Allocator (" << Name() << ") ran out of memory trying "
                 << "to allocate " << (num_bytes)
                 << ".  Current allocation summary follows.";
    DumpMemoryLog(rounded_bytes);
    std::cerr << RenderOccupancy();
  }
  return nullptr;
}
...
void BFCAllocator::SplitChunk(BFCAllocator::ChunkHandle h, size_t num_bytes) {
  ...
  Chunk* c = ChunkFromHandle(h);
  AT_ASSERT(!c->in_use() && (c->bin_num == kInvalidBinNum));
...
void BFCAllocator::DeallocateRawInternal(void* ptr) {
  if (ptr == nullptr) {
    std::cerr << "tried to deallocate nullptr";
    return;
  }
  std::lock_guard<std::mutex> l(lock_);
  ...
  AT_ASSERT(h != kInvalidChunkHandle);
...
void BFCAllocator::RemoveFreeChunkFromBin(BFCAllocator::ChunkHandle h) {
  Chunk* c = ChunkFromHandle(h);
  AT_ASSERT(!c->in_use() && (c->bin_num != kInvalidBinNum));
  AT_ASSERT(BinFromIndex(c->bin_num)->free_chunks.erase(h) > 0);
...
int64_t BFCAllocator::AllocationId(const void* ptr) {
  std::lock_guard<std::mutex> l(lock_);
  BFCAllocator::ChunkHandle h = region_manager_.get_handle(ptr);
  AT_ASSERT(h != kInvalidChunkHandle);
...
void RenderRegion(char* rendered, const size_t resolution,
  ...
  AT_ASSERT(start_location >= 0);
  AT_ASSERT(start_location < resolution);
  size_t end_location =
      ((ptr_c + size - 1 - base_ptr_c + offset) * resolution) /
      total_render_size;
  AT_ASSERT(end_location >= 0);
  AT_ASSERT(end_location < resolution);
...
void BFCAllocator::DumpMemoryLog(size_t num_bytes) {
  ...
  std::cerr << "Bin for " << (num_bytes)
            << " was " << (b->bin_size)
            << ", Chunk State: ";

  for (ChunkHandle h : b->free_chunks) {
    Chunk* c = ChunkFromHandle(h);
    std::cerr << c->DebugString(this, true);
  }
  ...
  for (const auto& region : region_manager_.regions()) {
    ...
    while (h != kInvalidChunkHandle) {
      ...
      std::cerr << (c->in_use() ? "Chunk" : "Free ") << " at " << c->ptr
                << " of size " << c->size;
      h = c->next;
    }
  }

  std::cerr << "     Summary of in-use Chunks by size: ";
  size_t total_bytes = 0;
  for (auto& it : in_use_by_size) {
    std::cerr << it.second << " Chunks of size " << it.first << " totalling "
              << (it.first * it.second);
    total_bytes += (it.first * it.second);
  }
  std::cerr << "Sum Total of in-use chunks: "
            << (total_bytes);
  std::cerr << "Stats: \n" << stats_.DebugString();
}
  ...
  for (const auto& region : region_manager_.regions()) {
    ...
    while (h != kInvalidChunkHandle) {
      ...
      if (c->in_use()) {
        ...
        AT_ASSERT(bin->free_chunks.count(h) == 1);
        AT_ASSERT(c->bin_num == bin_num);
      }
      h = c->next;
    }
  }
