...
auto PyFunction::legacy_apply(const variable_list& inputs) -> variable_list {
  ...
  for (size_t i = 0; i != inputs.size(); ++i) {
    PyObject* input;
    if (inputs[i].defined()) {
      input = createPyObject(data(inputs[i]));
...
std::pair<UnpackedInput, InputFlags> unpack_input(PyObject *args) {
  ...
  for (int i = 0; i < num_args; i++) {
    ...
    if (!is_variable) {
      ...
      unpacked.input_vars.push_back(Tensor(variable->cdata, true));
...
PyObject* process_outputs(THPFunction* grad_fn, const UnpackedInput& unpacked, THPObjectPtr&& raw_output, bool is_volatile) {
  ...
  t2var_type t2var;
  for (auto& var_tensor : unpacked.input_vars) {
    auto c_var = dynamic_cast<VariableTensor*>(var_tensor.get());
...
static PyObject *unpack_saved_variables(
    THPFunction *self,
    std::function<PyObject*(Tensor)> unpack_fn)
{
...
{
  return unpack_saved_variables(self, [](Tensor var) {
    auto pImpl = dynamic_cast<VariableTensor*>(var.get());
    return createPyObject(pImpl->data);
  });
}
