...
static Variable applySlice(const Variable& self, int64_t dim, PyObject* slice) {
...
static Type& getLongType(const Variable& v) {
  auto& baseType = v.data().type();
  return *VariableImpl::getType(baseType.toScalarType(kLong));
}

static Variable variableFromLongs(Type& type, int64_t size, long* data) {
  auto tensor = CPU(kLong).tensorFromBlob(data, { size });
  if (type.isCuda()) {
    return make_variable(tensor.toBackend(kCUDA));
  } else {
    return make_variable(tensor.clone());
  }
}
...
static Variable dispatchTakeIndex(const Variable& self, const variable_list& indices) {
  AutoNoGIL no_gil;
  AutoGPU auto_gpu(self);
  return self.index(asTensorList(indices));
}

static Variable dispatchPutIndex(Variable& self, const variable_list& indices, const Variable& value) {
  AutoNoGIL no_gil;
  AutoGPU auto_gpu(self);
...
PyObject* THPVariable_getitem(PyObject* self, PyObject* index) {
  ...
  return wrap(dispatchTakeIndex(sliced, variableIndices));
  Py_RETURN_NONE;
...
int THPVariable_setitem(PyObject* self, PyObject* index, PyObject* py_value) {
  ...
  dispatchPutIndex(sliced, variableIndices, value);
  return 0;
  END_HANDLE_TH_ERRORS_RET(-1)
}
