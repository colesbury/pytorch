...
AccumulateGrad::AccumulateGrad(Tensor _variable)
    : variable(_variable) {
  auto pImpl = dynamic_cast<VariableTensor*>(_variable.get());
  if (!pImpl) {
    throw std::runtime_error("AccumulateGrad(): expected VariableTensor");
  }
  is_executable = pImpl->requires_grad;
  num_inputs = 1;
}

auto AccumulateGrad::acc_inplace(Tensor grad, Tensor new_grad) -> void {
  AutoGPU guard(grad);

  auto& grad_data = data(grad);
  auto& new_grad_data = data(new_grad);

  if (grad_data.type().isSparse() && !new_grad_data.type().isSparse()) {
    grad_data = new_grad_data + grad_data;
  } else {
...
auto AccumulateGrad::apply(const variable_list& grads) -> variable_list {
  ...
  if (!var->grad.defined()) {
    ...
    if (new_grad_impl->is_volatile) {
      printf("making new_grad non-volatile\n");
      new_grad = Tensor(new VariableTensor(new_grad_impl->data), false);
    }
