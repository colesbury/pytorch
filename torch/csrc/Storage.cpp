...
PyObject* THPStorage_NewWithStorage(PyTypeObject* type, c10::Storage _storage) {
  ...
  TORCH_INTERNAL_ASSERT(storage_impl->pyobj_slot()->load_pyobj() == nullptr);
  storage_impl->pyobj_slot()->init_non_atomic(
      c10::impl::getGlobalPyInterpreter(), obj);
  storage_impl->init_pyobj(obj);
  return obj;
}
...
PyObject* THPStorage_Wrap(c10::Storage storage) {
  ...
  Py_INCREF(obj);
  if (!storage_impl->init_pyobj(obj)) {
    Py_DECREF(obj);
  }
  return obj;
}
