...
def _libnccl():
    global lib
    if lib is None:
        lib = ctypes.cdll.LoadLibrary('libnccl.so.2')
...
def communicator():
    global _comm
    if _uid is None:
        raise RuntimeError('NCCLv2 not initialized')
    if _comm is None:
...
def broadcast(tensor, root=0, stream=None):
    comm = communicator()
    if stream is not None:
        stream = stream.cuda_stream
    data_type = nccl_types[tensor.type()]
...
def get_device(comm):
    device = ctypes.c_int()
    check_error(lib.ncclCommCuDevice(
        comm,
        ctypes.byref(device)))
    return device.value
