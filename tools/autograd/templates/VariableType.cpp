...
VariableTensor* newVariableTensor(Type * baseType, const Tensor & tensor) {
  return new VariableTensor(tensor);
}

VariableTensor* newVariableTensor(Type * baseType, const Scalar & scalar) {
  if (scalar.isFloatingPoint()) {
    auto& doubleType = baseType->toScalarType(ScalarType::Double);
    auto tensor = doubleType.scalarTensor(scalar);
    return new VariableTensor(tensor.toType(*baseType));
  } else if (scalar.isIntegral()) {
    auto& longType = baseType->toScalarType(ScalarType::Long);
    auto tensor = longType.scalarTensor(scalar);
    return new VariableTensor(tensor.toType(*baseType));
  } else {
    return new VariableTensor(baseType->scalarTensor(scalar));
  }
}
